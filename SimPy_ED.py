...
class EmergencyDepartment:
    ...
    def __init__(self, num_doctors=3, num_nurses=5, num_CTs=2, num_lab=2, sim_time=12*60, seed=None):
        # TODO
        if seed is not None:
            random.seed(seed)
    ...
    def calculate_avg_los(self):
        return np.average(self.los_total) / 60
    ...
    def calculate_avg_weighted_waiting_time(self):
        ...
        for k in range(len(self.waiting_time_au)):
            ...
            if len(self.waiting_time_au[key]) != 0:
                sum += 0.1 * (5 - k) * np.average(self.waiting_time_au[key]) / 60
    ...
    def calculate_avg_weighted_los(self):
        ...
        for k in range(len(self.los_au)):
                ...
                if len(self.los_au[key]) != 0:
                    sum += 0.1 * (5 - k) * np.average(self.los_au[key]) / 60
        return sum
