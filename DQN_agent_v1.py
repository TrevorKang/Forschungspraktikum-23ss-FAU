...
class AgentDQN:
    ...
    def __init__(self, env, replay_size=10000, batch_size=64, gamma=0.99, sync_after=10, lr=0.001):
        ...
        :param env: the environment which the agent should learn in --> gym.Environment
        :param replay_size: size of replay buffer --> int
        :param batch_size: number of samples from replay buffer that should be performed in optimization step --> int
        :param gamma: discount factor --> float
        :param sync_after: the time steps after which the target network should be synchronized with the main --> int
        :param lr: learning rate, the learning rate for Adam Optimizer --> float
        """
        ...
        """
        Soft update to the weights, maintains a fixed value during the learning process of the original Q-network,
        and then periodically resets it to the original Q-network value.
        This can be effective because the Q-network can be approached with a fixed target network.
        """
    ...
    def learn(self, timesteps):
        ...
        for timestep in range(1, timesteps + 1):
            ...
            if timestep == 20000:
                plt.savefig('./Rewards/DQN_rewards.png')
...
if __name__ == '__main__':
    ...
    dqn = AgentDQN(env, batch_size=100, gamma=0.9999)
    dqn.learn(20000)
    print()